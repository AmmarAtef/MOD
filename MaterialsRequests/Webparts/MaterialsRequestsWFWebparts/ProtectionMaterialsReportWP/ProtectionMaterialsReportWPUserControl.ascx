...
<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="ProtectionMaterialsReportWPUserControl.ascx.cs" Inherits="MaterialsRequestsWFWebparts.ProtectionMaterialsReportWP.ProtectionMaterialsReportWPUserControl" %>
                    ...
                    <thead>
                        <tr class="active">
                            ...
                            <th class="col-xs-2"><strong>
                                <asp:Label runat="server" ID="GridTitle_RequestEnterDatePrint" ClientIDMode="Static">
                                </asp:Label>

                            </strong></th>
                            <th class="col-xs-2">عدد المواد</th>
                        </tr>
                    </thead>
                    <tbody id="tablebody">
                    </tbody>
            ...
            <table id="dataTable" class="table  ">
                <thead>
                    <tr class="active">
                        ...
                        <th class="col-xs-2"><strong>تاريخ الطلب</strong></th>
                        <th class="col-xs-2"><strong>إسم المندوب </strong></th>
                        <th class="col-xs-2"><strong>إسم مقدم الطلب</strong></th>
                        <th class="col-xs-2"><strong>إسم مندوب الأمن</strong></th>
                        <th class="col-xs-2"><strong>
...
<script type="text/javascript">
    ...
    function renderSearchResult(data) {
        ...
        for (var x = 0; x < requestsArrayData.length; x++) {
            ...
            for (var i = 0; i < requestsArrayData[x].RequestMaterials.length; i++) {
                for (var j = 0; j < requestsArrayData[x].RequestMaterials[i].MaterialActions.length; j++) {
                    checkInArr.push(requestsArrayData[x].RequestMaterials[i].MaterialActions[j].ActionDate);
                }
                var lastCheckIn = checkInArr.pop();
                var lastCheckInTime = requestType == "0" ? "لم يتم دخول المادة حتى الآن" : "لم يتم خروج المادة حتى الآن";
                ...
                //if (requestsArrayData[x].RequestMaterials[i].MaterialActions.length > 0) {
                if (requestsArrayData[x].RequestType == 0) {
                    RequestTypeStatus = "In";
                }
                //innerDivs += '<div class="col-md-4"><div class="inner-card accept-request"><span id="cardMaterialID" style="display:none">' + requestsArrayData[x].RequestMaterials[i].ID + '</span><p id="cardMaterialName" class="title"> اسم الصنف :' + requestsArrayData[x].RequestMaterials[i].MaterialName + '</p><p id="cardMaterialQuantity" class="Quantity"> العدد : ' + requestsArrayData[x].RequestMaterials[i].Quantity + '</p><p><span>' + (requestType == "0" ? " وقت الدخول" : " وقت الخروج   ") + '</span><span>:</span><span class="check-time">' + lastCheckInTime + '</span></p></div></div>';
                ...
                if (requestsArrayData[x].RequestMaterials[i].MaterialActions != null && requestsArrayData[x].RequestMaterials[i].MaterialActions.length > 0) {
                    protectionPerson = '<p id="protectionManName" class="Quantity"> مندوب الحماية : ' + requestsArrayData[x].RequestMaterials[i].MaterialActions[0].ActionDoneByName + '</p>'
                }
                innerDivs += '<div class="col-md-4"><div class="inner-card accept-request"><span id="cardMaterialID" style="display:none">' + requestsArrayData[x].RequestMaterials[i].ID + '</span><p id="cardMaterialName" class="title"> اسم الصنف :' + requestsArrayData[x].RequestMaterials[i].MaterialName + '</p><p id="cardMaterialQuantity" class="Quantity"> العدد : ' + requestsArrayData[x].RequestMaterials[i].Quantity + '</p><p><span>' + (requestType == "0" ? " وقت الدخول" : " وقت الخروج   ") + '</span><span>:</span><span class="check-time">' + lastCheckInTime + '</span></p>' + protectionPerson + '</div></div>';
    ...
    $(function () {
        ...
        if ($("#hdnRequestType").val() == "0") {
            document.getElementById("GridTitle_RequestEnterDate").innerHTML = document.getElementById("GridTitle_RequestEnterDatePrint").innerHTML = "<sharepoint:encodedliteral runat='server' text='<%$Resources:MaterialsRequests ,MaterialEnterDate%>' EncodeMethod='EcmaScriptStringLiteralEncode'/>";


        } else {

            document.getElementById("GridTitle_RequestEnterDate").innerHTML = document.getElementById("GridTitle_RequestEnterDatePrint").innerHTML = "<sharepoint:encodedliteral runat='server' text='<%$Resources:MaterialsExitRequests ,MaterialEnterDate%>' EncodeMethod='EcmaScriptStringLiteralEncode'/>";
    ...
    function getRequests() {
        ...
        var searchRequestsAPI$ = apiRootAddress + "MaterialsRequest/GetActions?ListSiteUrl=" + listSiteURL + "&RequestID=0&ToDay=false&UserID=0&RequestCreateDate=" + requestCreationDate + "&Gate=&ApplicantName=&SupervisorName=&MaterialName=&DepartmentCode=&SectionCode=&RequestType=" + $("#hdnRequestType").val();
    ...
    function searchRequest() {
        ...
        var searchRequestsWithFilterationsAPI$ = apiRootAddress + "MaterialsRequest/GetActions?ListSiteUrl=" + listSiteURL + "&RequestID=0&ToDay=false&UserID=0&RequestCreateDate=" + requestCreationDate + "&Gate=" + gate + "&ApplicantName=" + applicantName.trim() + "&SupervisorName=" + supervisorName.trim() + "&MaterialName=" + matrialName.trim() + "&DepartmentCode=" + departmentCode + "&SectionCode=" + sectionCode + "&RequestType=" + $("#hdnRequestType").val();
